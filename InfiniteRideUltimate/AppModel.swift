...
final class AppModel: ObservableObject {
    ...
    var displayedPower: Int {
        if power > 0 { return power }
        return samples.last?.power ?? 0
    }
    ...
    private func capture() {
        ...
        let rawModeledPower = modeledPower(speed: currentSpeedMps, grade: grade)
        if power > 0 { learnPowerCalibration(realWatts: power, modeledWatts: rawModeledPower) }
        let recordedPower = power > 0 ? power : calibratedPower(rawModeledPower)
        samples.append(RideSample(date: now, elapsed: elapsed, distanceMeters: distanceMeters, speedMps: currentSpeedMps, heartRate: heartRate, cadence: cadence, power: recordedPower, altitudeMeters: altitude, grade: grade, coordinate: loc.map { Coordinate($0.coordinate) }, powerMeasured: power > 0))
    ...
    private func calibratedPower(_ rawWatts: Int) -> Int {
        let mode = profile.powerCalibrationMode ?? .automatic
        guard mode != .off, rawWatts > 0 else { return rawWatts }
        let factor = min(2.0, max(0.5, profile.powerCalibrationFactor ?? 1.0))
        let offset = mode == .manual ? min(150, max(-150, profile.powerCalibrationOffsetWatts ?? 0)) : 0
        return max(0, Int((Double(rawWatts) * factor).rounded()) + offset)
    }

    private func learnPowerCalibration(realWatts: Int, modeledWatts: Int) {
        guard (profile.powerCalibrationMode ?? .automatic) == .automatic,
              realWatts >= 30, modeledWatts >= 30, currentSpeedMps >= 2 else { return }
        let observed = min(2.0, max(0.5, Double(realWatts) / Double(modeledWatts)))
        let samples = max(0, profile.powerCalibrationSamples ?? 0)
        let alpha = samples < 30 ? 0.08 : 0.02
        let current = min(2.0, max(0.5, profile.powerCalibrationFactor ?? 1.0))
        profile.powerCalibrationFactor = current * (1.0 - alpha) + observed * alpha
        profile.powerCalibrationSamples = min(100_000, samples + 1)
        profile.powerCalibrationUpdatedAt = Date()
        if samples % 10 == 9 { save() }
    }

    func resetPowerCalibration() {
        profile.powerCalibrationFactor = 1.0
        profile.powerCalibrationOffsetWatts = 0
        profile.powerCalibrationSamples = 0
        profile.powerCalibrationUpdatedAt = nil
        saveProfile()
        message = "Power calibration reset"
    }
