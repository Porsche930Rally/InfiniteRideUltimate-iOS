...
private struct BottomBar:View{
    @EnvironmentObject var app:AppModel
    var body:some View{HStack{ForEach(AppTab.allCases,id:\.self){item in Button{app.tab=item}{VStack(spacing:4){Image(systemName:symbol(item)).font(.system(size:18));Text(item.rawValue.capitalized).font(.caption2.weight(.semibold))}.foregroundStyle(app.tab==item ? IRTheme.lime:Color.white.opacity(.62)).frame(maxWidth:.infinity)}}}.padding(.top,9).padding(.bottom,7).background(.ultraThinMaterial).overlay(alignment:.top){Rectangle().fill(IRTheme.line).frame(height:1)}}
