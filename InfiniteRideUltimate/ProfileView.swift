...
struct BikeShowroomView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                bike(at: selection.index - 1, width: geo.size.width, height: geo.size.height).scaleEffect(0.72).opacity(0.35).offset(x:-geo.size.width * 0.35)
                bike(at: selection.index + 1, width: geo.size.width, height: geo.size.height).scaleEffect(0.72).opacity(0.35).offset(x:geo.size.width * 0.35)
                ...
                RoundedRectangle(cornerRadius:23).stroke(IRTheme.cyan.opacity(0.7),lineWidth:1.5)
    ...
    private func bike(at raw:Int,width:CGFloat,height:CGFloat)->some View {
        ...
        return Image("bike_chooser_showroom").resizable().scaledToFill()
            .frame(width:width*CGFloat(count),height:height).offset(x:(CGFloat(count-1)/2-CGFloat(index))*width)
            .frame(width:width,height:height).clipped().allowsHitTesting(false)
    }
    private func move(_ amount:Int){let all=BikeType.allCases,index=(selection.index+amount+all.count)%all.count;withAnimation(.easeOut(duration:0.23)){selection=all[index]}}
}
